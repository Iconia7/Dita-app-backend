    ...
    def update_game_stats(self, request):
        ...
        if game_type == "snake":
            ...
            points_earned = request.data.get(
                "points", 0
            )  # RAM points are more complex, so we accept them from frontend
...
class StudyGroupViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = StudyGroup.objects.annotate(
            is_member=Exists(StudyGroup.objects.filter(id=OuterRef("pk"), members=user)), member_count=Count("members")
        )
        return queryset
